...
m2 <- read_delim("Data/cleanData.csv", 
    ";", escape_double = FALSE, trim_ws = TRUE)
```
...
```{r}
set.seed(100)
dataPartition = sample(2,nrow(m2),replace=TRUE,prob=c(0.7,0.3))
...
m2pred <- prediction(m2test$lr_score, m2test$DefaultedLoans)
m2perf <- performance(m2pred,"tpr","fpr")



#ROC
plot(m2perf, lwd=2, colorize=TRUE, main="ROC LR: Logistic Regression Performance")
...
##KS, Gini & AUC m2
```{r}
lr_KS <- round(max(attr(m2perf,'y.values')[[1]]-attr(m2perf,'x.values')[[1]])*100, 2)
lr_AUROC <- round(performance(m2pred, measure = "auc")@y.values[[1]]*100, 2)
...
#Data Subsetting
set.seed(100)
dataPartition = sample(2,nrow(m1),replace=TRUE,prob=c(0.7,0.3))
...
m1pred <- prediction(m1test$lr_score, m1test$DefaultedLoans)
m1perf <- performance(m1pred,"tpr","fpr")
...
##KS, Gini & AUC m1
```{r}
lr_KS <- round(max(attr(m1perf,'y.values')[[1]]-attr(m1perf,'x.values')[[1]])*100, 2)
lr_AUROC <- round(performance(m1pred, measure = "auc")@y.values[[1]]*100, 2)
lr_Gini <- (2*lr_AUROC - 100)
cat("AUROC: ",lr_AUROC,"\tKS: ", lr_KS, "\tGini:", lr_Gini, "\n")
```
...
```{r}
set.seed(100)
dataPartition = sample(2,nrow(m3),replace=TRUE,prob=c(0.7,0.3))
...
m3pred <- prediction(m3test$lr_score, m3test$DefaultedLoans)
m3perf <- performance(m3pred,"tpr","fpr")
...
DT_prediction <- prediction(m2test$DT_score[,2],m2test$DefaultedLoans)
...
DT_prediction <- prediction(m1test$DT_score[,2],m1test$DefaultedLoans)
DT_performance <- performance(DT_prediction,"tpr","fpr")
...
DT_prediction <- prediction(m3test$DT_score[,2],m3test$DefaultedLoans)
DT_performance <- performance(DT_prediction,"tpr","fpr")
